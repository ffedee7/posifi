...
if SETTINGS:
    ...
    MACS_5GHZ = settings['MACS_5GHZ'].split(',') if FILTER else []
    MACS_2_4GHZ = settings['MACS_2_4GHZ'].split(',') if FILTER else []
else:
    FILTER = os.environ['FILTER_MACS']
    FINGERPRINT_NULL_VALUE = int(os.environ['FINGERPRINT_NULL_VALUE'])
    MACS_5GHZ = os.environ['MACS_5GHZ'].split(',') if FILTER else []
    MACS_2_4GHZ = os.environ['MACS_2_4GHZ'].split(',') if FILTER else []
...
class AIEngine():
    ...
    def get_datasets(self):
        ...
        dynamo_data = dynamo_data.drop(columns=["result", 'timestamp'])

        filtered_macs = MACS_5GHZ + MACS_2_4GHZ if self.is_5ghz else MACS_2_4GHZ

        X = dynamo_data.reindex(
            columns=filtered_macs) if FILTER else dynamo_data

        X = X.fillna(FINGERPRINT_NULL_VALUE)
    ...
    def train(self):
        ...
        for model_name in self.algorithms.keys():
            self.youden_indexes[model_name] = self.youden_statistic(
                y_test, results[model_name])
    ...
    def classify(self, X_val):
        ...
        for i, model_name in enumerate(self.algorithms.keys()):
            threads[i] = Thread(
                ...
                args=(X_val, model_name,
                      self.algorithms[model_name], model_results)
            )
            threads[i].start()
        ...
        for model_name, model_result in model_results.items():
            probabilities = probabilities + model_result * \
                self.youden_indexes[model_name]
